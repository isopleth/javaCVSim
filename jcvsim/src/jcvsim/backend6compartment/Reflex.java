...
public class Reflex {
    ...
    public static final double S_GRAN = 0.0625;// Signal history bin size
    public static final double S_INT = 0.25;    // Averaging interval chosen such that 4 bins fit into
    // one interval of size S_INT
    public static final int S_LENGTH = 4;    // Number of bins of size S_GRAN to fit into averaging
    ...
    public static double sanode(Data_vector p, Reflex_vector r, Parameter_vector theta, double dt) {
        ...
        if (f_new >= 60.0) {
            ...
            p.time_new[2] = theta.get(PVName.PV118) * sqrt(t_onset);  // PR-interval
            p.time_new[3] = theta.get(PVName.PV119) * sqrt(t_onset);  // atrial systole
            p.time_new[4] = theta.get(PVName.PV120) * sqrt(t_onset);  // ventricular systole
        }
        ...
        if ((f_new >= 60.0) && ((a_time - PR_old) > 1.5 * Tv_old)) {
            ...
            // Update the cardiac timing variables for the next beat to be taken.
            p.time[2] = theta.get(PVName.PV118) * sqrt(t_onset);  // PR-interval
            p.time[3] = theta.get(PVName.PV119) * sqrt(t_onset);  // atrial systole
 ...
 /*
    ...
    public static void makeImp(Impulse_vector imp, Parameter_vector theta) {
        ...
        start = (int) rint((theta.get(PVName.PARASYMP_DELAY) - 3.0 * S_GRAN) / S_GRAN);
        peak = (int) rint((theta.get(PVName.PARASYMP_PEAK) - 3.0 * S_GRAN) / S_GRAN);
        end = (int) rint((theta.get(PVName.PARASYMP_END) - 3.0 * S_GRAN) / S_GRAN);
        ...
        start = (int) rint((theta.get(PVName.PV112) - 3.0 * S_GRAN) / S_GRAN);
        peak = (int) rint((theta.get(PVName.PV113) - 3.0 * S_GRAN) / S_GRAN);
        end = (int) rint((theta.get(PVName.PV114) - 3.0 * S_GRAN) / S_GRAN);
        ...
        start = (int) rint((theta.get(PVName.PV115) - 3.0 * S_GRAN) / S_GRAN);
        peak = (int) rint((theta.get(PVName.PV116) - 3.0 * S_GRAN) / S_GRAN);
        end = (int) rint((theta.get(PVName.PV117) - 3.0 * S_GRAN) / S_GRAN);
        ...
        start = (int) rint((theta.get(PVName.CPR_VEN_DELAY_ALPHA) - 3.0 * S_GRAN) / S_GRAN);
        peak = (int) rint((theta.get(PVName.CPR_VEN_PEAK_ALPHA) - 3.0 * S_GRAN) / S_GRAN);
        end = (int) rint((theta.get(PVName.CPR_VEN_END_ALPHA) - 3.0 * S_GRAN) / S_GRAN);
        ...
        start = (int) rint((theta.get(PVName.CPR_ART_DELAY_ALPHA) - 3.0 * S_GRAN) / S_GRAN);
        peak = (int) rint((theta.get(PVName.CPR_ART_PEAK_ALPHA) - 3.0 * S_GRAN) / S_GRAN);
        end = (int) rint((theta.get(PVName.CPR_ART_END_ALPHA) - 3.0 * S_GRAN) / S_GRAN);
    ...
    public static void queue_ptr(Data_vector p, Impulse_vector imp, Reflex_vector r,
        ...
        if (p.time[0] < S_INT) {
            if ((S_cum_dt + dt) < S_GRAN) {
                ...
                S_cum_rap += (p.pressure[CENTRAL_VENOUS_CPI] - p.pressure[INTRA_THORACIC_CPI]) * dt;
                S_cum_abp += p.pressure[ARTERIAL_CPI] * dt;
            } else {
                // Account for the fraction of the last time step that fell within
                // the averaging interval - again: Backward Euler integration.

                S_cum_rap += (p.pressure[CENTRAL_VENOUS_CPI] - p.pressure[INTRA_THORACIC_CPI]) * (S_GRAN - S_cum_dt);
                S_cum_abp += p.pressure[ARTERIAL_CPI] * (S_GRAN - S_cum_dt);
                ...
                s_rap = atan((I_cum_rap - theta.get(PVName.ABR_HR_SYMPATHETIC_GAIN)) / theta.get(PVName.CPR_SCALING_FACTOR))
                ...
                S_cum_rap = (p.pressure[CENTRAL_VENOUS_CPI] - p.pressure[INTRA_THORACIC_CPI]) * S_cum_dt;
                S_cum_abp = p.pressure[ARTERIAL_CPI] * S_cum_dt;
            }
        ...
        r.compliance[0] = theta.get(PVName.RV_SYSTOLIC_COMPLIANCE) + theta.get(PVName.ABR_CONTRACT_RV_SYMP_GAIN) * beta_resp;
        r.compliance[0] = (r.compliance[0] > 0.3 ? r.compliance[0] : 0.3);

        r.compliance[1] = theta.get(PVName.LV_SYSTOLIC_COMPLIANCE) + theta.get(PVName.ABR_CONTRACT_LV_SYMP_GAIN) * beta_resp;
        r.compliance[1] = (r.compliance[1] > 0.01 ? r.compliance[1] : 0.01);

        // Peripheral resistance feedback
        r.resistance[0] = theta.get(PVName.TOTAL_PERIPHERAL_RESISTANCE) + theta.get(PVName.ABR_ART_RESISTANCE_SYMP_GAIN) * alpha_resp
                + theta.get(PVName.CPR_ART_RESISTANCE_SYMP_GAIN) * alphav_resp;
        ...
        r.volume[0] = theta.get(PVName.KIDNEY_COMPARTMENT_ZPFV) + theta.get(PVName.SPLAN_COMPARTMENT_ZPFV) + theta.get(PVName.LBODY_COMPARTMENT_ZPFV)
                + theta.get(PVName.ABDOM_VEN_ZPFV) + theta.get(PVName.INFERIOR_VENA_CAVA_ZPFV) + theta.get(PVName.SUPERIOR_VENA_CAVA_ZPFV)
                + theta.get(PVName.ABR_VEN_TONE_SYMPATHETIC_GAIN) * alpha_respv + theta.get(PVName.CPR_VEN_TONE_SYMP_GAIN) * alphav_respv;

        p.pressure[CENTRAL_VENOUS_CPI] += (Vold - r.volume[0]) / theta.get(PVName.VEN_COMPLIANCE);
    }
